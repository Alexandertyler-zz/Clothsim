...
//Vectors containing our particle system and constraint system
    ...
    for (int i=0; i<numSpheres; i++) {
        ...
        if (i%2 == 0 && i!=0) {
            ...
            if (dist < radius) {//-0.00001) {
                collision = true;
                glm::vec3 posForNorm = (pos-(glm::vec3(0, sphereVert, sphereTranslation * offset + sphereHoriz)));//take pos in relation to origin at (0,0,0) to use glm::normalize
                ...
                pos = pos + (glm::vec3(0, sphereVert, sphereTranslation * offset + sphereHoriz));//re-offset so pos is set back in relation to own sphere's origin
